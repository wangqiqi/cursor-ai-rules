...
5. skill-dispatcher/SKILL.md
   ...
   → 加载: .cursor/features/skills/api-design.md
6. 应用技能指导
   → 生成API设计方案
   → RESTful建议
   → 安全考虑
...
第一步: 调用skill-dispatcher
    ...
    ↓
第二步: 技能发现
    文件: .cursor/features/skills/registry.json
    动作:
    - 读取注册表
    - 解析技能元数据
    - 获取分类和依赖
...
4. 依赖检查
   ...
   ✓ java - 已检测到
5. 加载技能
   文件: .cursor/features/skills/backend-development.md
6. 应用技能指导
   → 微服务架构设计
   → 服务拆分建议
   → 通信模式推荐
...
第一步: 识别角色请求
    ...
    ↓
第二步: 角色系统处理
    文件: .cursor/core/role-manager.sh
    动作:
    - 验证角色存在
    - 检查昵称映射
    - 加载角色配置
