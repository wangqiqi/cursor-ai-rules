...
*版本: v4.3.0 | 最后更新: 2026-01-17 | 作者: wangqiqi (https://github.com/wangqiqi)*

## 🧠 核心理念：全资源统一指挥 + 智能感知 + 自动决策 + 自主执行

**🚀 升级完成：真正的单一入口系统 + VIBE开发集成**
- ✅ **39个脚本**：100%覆盖所有可执行资源
- ✅ **57个意图映射**：支持复杂多样的用户需求（新增VIBE开发支持）
- ✅ **23个规则**：完整规则系统集成（含VIBE Coding原则）
- ✅ **24个技能**：全专业技能库调用
- ✅ **6大VIBE服务**：智能VIBE开发流程支持
- ✅ **开箱即用**：复制到任意项目，零配置启动

**颠覆传统命令模式**：不再需要用户记忆复杂的命令语法和参数，AI通过智能感知自动理解需求并执行最合适的操作组合。同时深度集成VIBE Coding开发方法论，提供文档驱动、测试先行、前后端对齐的智能化开发体验。
...
#### 🎯 VIBE开发系列
```bash
@master 使用VIBE开发项目         # 启动VIBE Coding开发流程
@master VIBE项目创建 [描述]       # 基于VIBE原则的项目初始化
@master VIBE需求分析 [需求]       # VIBE产品需求文档生成
@master VIBE代码生成 [功能]       # VIBE智能代码生成
@master VIBE测试驱动 [场景]       # VIBE测试驱动开发
@master VIBE接口对齐             # VIBE前后端对齐验证
@master VIBE质量检查             # VIBE质量门禁验证
@master VIBE部署配置             # VIBE生产环境部署
```
...
# 输入过程中系统会智能提示：
@master 创建
# 系统提示: "检测到项目创建意图，建议: React/Vue/Node/Python/全栈/VIBE开发"

@master VIBE
# 系统提示: "检测到VIBE开发意图，支持: 项目创建/需求分析/代码生成/测试驱动/接口对齐/质量检查/部署配置"
...
@master VIBE接口对齐 --fix auto  # 自动修复对齐问题
```
...
# VIBE开发偏好学习：
@master VIBE项目创建  # 第一次VIBE开发
# 系统记录: 用户偏好React + Node.js + PostgreSQL + Playwright

@master VIBE代码生成  # 后续使用
# 系统自动应用: React组件 + TypeScript + 设计系统 + 单元测试
...
#### VIBE开发模式
```bash
# 直接通过@master调用VIBE开发流程
@master 使用VIBE开发项目         # 启动完整的VIBE开发模式
@master VIBE项目创建 电商平台     # 基于VIBE原则的项目初始化
@master VIBE需求分析 用户管理系统 # 生成VIBE风格的PRD文档
@master VIBE代码生成 登录功能     # VIBE驱动的代码生成
@master VIBE测试驱动 支付流程     # 测试先行的开发模式
@master VIBE接口对齐             # 自动前后端对齐验证
@master VIBE质量检查             # VIBE质量门禁检查
@master VIBE部署配置             # 生产就绪的部署配置
```

#### VIBE执行模式
```bash
@master VIBE项目创建 --mode standard   # 标准VIBE开发模式
@master VIBE项目创建 --mode enterprise # 企业级VIBE开发模式
@master VIBE质量检查 --depth full      # 完整的质量验证
@master VIBE接口对齐 --auto-fix        # 自动修复对齐问题
```
...
基于感知结果，AI会：
1. **识别需求类型** - 项目创建/代码优化/部署运维/技术学习/VIBE开发
2. **评估项目状态** - 新项目/成熟项目/重构项目/VIBE开发阶段
3. **选择最佳方案** - 推荐最适合的技术栈和工具组合/VIBE开发流程
4. **执行操作序列** - 按正确顺序自动执行所需命令/VIBE开发阶段
5. **提供反馈建议** - 实时反馈执行状态和后续建议/VIBE质量评分
