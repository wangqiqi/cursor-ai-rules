...
> **目录关系**:
> - **Cursor 可发现技能包**: `.cursor/skills/<name>/SKILL.md`（标准 frontmatter；含 `skill-dispatcher`、`master`、`api-design` 等）
> - **技能库（正文 + 元数据）**: `.cursor/features/skills/skills/`（`registry.json` + flat `.md` 文件）
>
> 优先读取 `.cursor/skills/*/SKILL.md`；需要完整步骤时再打开 `registry.json` 中 `path` 指向的 `.md`。
...
读取 .cursor/features/skills/skills/backend-development.md
✓ 技能已加载，现在可以提供后端开发指导
```
...
- **技能注册表**: `.cursor/features/skills/skills/registry.json`
