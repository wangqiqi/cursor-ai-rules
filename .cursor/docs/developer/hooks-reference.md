...
| 系统 | 版本 | 配置文件路径 |
|------|------|-------------|
| **Cursor 原生 Hooks** | v1 | `.cursor/features/hooks/hooks.json` |
| **Hooks 引擎** | v3 | `.cursor/features/hooks/hooks.json` |
...
- **Cursor 原生钩子**（来自 `.cursor/features/hooks/hooks.json`）部分字段未在配置中显式定义，表中以 `—` 表示。
