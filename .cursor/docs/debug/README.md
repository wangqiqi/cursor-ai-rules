...
```bash
# 1. 确保脚本有执行权限
chmod +x .cursor/core/isolation-debugger.sh
chmod +x .cursor/core/pattern-analyzer.sh

# 2. 查看帮助信息
.cursor/core/isolation-debugger.sh help
.cursor/core/pattern-analyzer.sh help
```
...
```bash
# 隔离可疑的JavaScript文件
.cursor/core/isolation-debugger.sh isolate src/utils.js

# 指定隔离深度和测试命令
.cursor/core/isolation-debugger.sh isolate src/utils.js --depth 3 --test-cmd "npm run test:unit"

# 使用条件编译隔离
.cursor/core/isolation-debugger.sh isolate config.js --type conditional
```

#### 恢复隔离的文件
```bash
# 恢复最新的备份
.cursor/core/isolation-debugger.sh restore src/utils.js

# 恢复指定时间点的备份
.cursor/core/isolation-debugger.sh restore src/utils.js --backup 20260116_143000
```

#### 分析文件结构
```bash
# 查看文件可隔离的代码块
.cursor/core/isolation-debugger.sh analyze src/complex-module.js
```
...
# 分析特定目录
.cursor/core/pattern-analyzer.sh analyze --source src/

# 保存分析结果
.cursor/core/pattern-analyzer.sh analyze --output error_analysis.json
```

#### 批量修复相似错误
```bash
# 修复调试日志 (安全操作)
.cursor/core/pattern-analyzer.sh batch-fix "console.log"

# 修复TODO注释
.cursor/core/pattern-analyzer.sh batch-fix "TODO"

# 预览修复效果 (不实际修改)
.cursor/core/pattern-analyzer.sh batch-fix "console.log" --dry-run

# 限制修复文件数量
.cursor/core/pattern-analyzer.sh batch-fix "undefined" --max-files 5
```

#### 生成分析报告
```bash
# 生成HTML格式的详细报告
.cursor/core/pattern-analyzer.sh report error_analysis.json --format html

# 生成文本格式的简洁报告
.cursor/core/pattern-analyzer.sh report error_analysis.json --format text
```

#### 验证修复效果
```bash
# 验证修复的实际效果
.cursor/core/pattern-analyzer.sh validate-fix "console.log"

# 使用自定义测试命令
.cursor/core/pattern-analyzer.sh validate-fix "undefined" --test-cmd "jest --coverage"
```
...
```bash
# 清理7天前的备份
.cursor/core/isolation-debugger.sh cleanup --retention 7
```
  ...
  debug-analysis:
    ...
    steps:
      ...
      - name: Run Debug Analysis
        run: .cursor/core/pattern-analyzer.sh analyze --output debug-report.json
  ...
  "commands": [
    {
      ...
      "command": ".cursor/core/isolation-debugger.sh isolate $FILE"
    },
    {
      "name": "pattern-analyze",
      "description": "分析错误模式",
      "command": ".cursor/core/pattern-analyzer.sh analyze"
    }
  ]
}
```
