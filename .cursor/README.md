...
- ✅ **审计追踪日志**: 所有交互结构化日志存储在 `.cursorGrowth/` 中
- ✅ **决策路径可视化**: 显示从输入到输出的完整决策路径
- ✅ **历史追溯性**: 支持对任何历史会话的完整回顾

## 🤝 核心协作原则

基于三项基本公理，创造**人机共生**协作模式，确保AI始终服务于人类意图和利益。
...
### ⚡ Token优化系统 (25-35%节省)
- **多层压缩算法**: 支持minimal/balanced/aggressive/maximum四级压缩
- **流式输出机制**: 分块输出，避免一次性大量响应，减少网络开销
- **上下文缓存优化**: 智能缓存上下文信息，避免重复传递项目数据
- **装饰字符自动移除**: 自动清理emoji和冗余格式字符
- **实时Token监控**: 智能预警系统，超过阈值自动提醒优化
- **性能缓存系统**: 统一的缓存基础设施，支持内存/文件/网络三层架构
- **精简输出模式**: 默认启用精简模式，减少不必要的输出内容
...
- **🔒 隐私保护**: `.cursorGrowth` 数据完全私有且不共享
- **👥 协作友好**: 团队共享配置，每个人数据保持独立
...
### 工作流集成与规则系统
- **规则应用**: 使用 `.cursor/rules` 进行专业指导，根据上下文和文件类型应用规则
- **智能功能**: 组合多个规则进行全面覆盖，自适应规则优化
...
| 功能                                     | 描述                                                                       | 效果                                |
| ---------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------- |
| 🧠 **统一智能命令入口**                   | `@master` 用自然语言唤醒所有能力，AI驱动协作                             | **零记忆负担**                      |
...
| 🎭 **个性角色系统**                       | 21种不同AI人格，支持昵称呼叫，独特的交互风格                                | **个性化体验**                      |

## 🎭 个性角色系统

### 21种AI人格 - 个性化交互体验

角色系统提供**21种独特的AI人格**，每种都有独特的交互风格、语言模式和行为特征，让你可以根据心情、任务类型和个人偏好选择最适合的AI伙伴。现在还支持**角色昵称功能**，让呼叫变得更加亲切自然！

#### 🤖 标准专业角色 (8种)

| 角色ID | 角色名称 | 特色风格 | 适用场景 |
|--------|----------|----------|----------|
| `professional_assistant` | 专业助手 | 正式、平衡、高效 | 工作环境，默认角色 |
| `humble_assistant` | 谦逊助手 | 礼貌、尊重、细致 | 正式场合、商务沟通 |
| `friendly_partner` | 友好伙伴 | 随和、温暖、协作 | 日常开发、团队协作 |
| `expert_mentor` | 专家导师 | 权威、博学、指导 | 学习成长、技术培训 |
| `creative_artist` | 创意艺术家 | 创新、灵感、艺术化 | 创意开发、设计项目 |
| `strict_teacher` | 严格老师 | 规范、严格、纪律性 | 代码审查、质量控制 |
...
| `perfect_maid` | 完美女仆 | 终极优雅、极致服务、绝对忠诚 | 高端服务、完美体验 |
...
/master 切换角色 perfect_maid            # 完美女仆模式
...
| **个性角色系统**                   | 21种不同AI人格系统                         | 专业/谦逊/友好/专家/创意/严格/幽默/极简 + 动漫风格 + 昵称呼叫功能                | ✅ 生产就绪     |
...
### 自定义规则
- **规则创建**: 在 `.cursor/rules/` 下创建新规则
- **规则配置**: 使用Markdown格式定义规则逻辑
- **规则激活**: 自动或手动激活规则
- **规则优先级**: 配置规则执行优先级
...
##### **配置数据 (config/)**
- `project/`: 项目特定配置和设置
- `environment/`: 环境检测结果和适配
- `customization/`: 用户自定义设置

##### **外部集成 (integrations/)**
- `api/`: API调用历史和缓存
- `webhooks/`: Webhook配置和日志
- `third_party/`: 第三方服务集成数据

##### **研究与实验 (research/)**
- `experiments/`: A/B测试和实验结果
- `feedback/`: 用户反馈和建议
- `iterations/`: 系统迭代和改进记录
