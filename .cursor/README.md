...
### ⚡ Token 优化（索引 + 精简为主）
- **`.cursorignore`**：排除 `references/`、测试与生长数据，减小 `@codebase` 索引（实测更有效）
- **文本压缩脚本**：`compress-markdown-text.py` + `token-compression.sh`（README 类文档实测约 **15%** token↓，见 [benchmark](docs/reference/TOKEN_COMPRESSION_BENCHMARK.md)）
- **流式输出 / 上下文缓存**：降低重复传输体积
- **实时监控**：`token-monitor.js` + 可选 tiktoken 基准
...
| 初始化时间 | ~30s | ~3s | **约 90%↑** |
| 感知时间 | ~10s | ~0.5s | **约 95%↑** |
| Token 节省（脚本/索引） | 视项目而定 | 见 benchmark | **请自测**，无固定比例承诺 |
| 组件数量 | 42 | 161+ | **约 283%↑** |
| 系统稳定性 | 95% | 99.9% | **设计目标** |
