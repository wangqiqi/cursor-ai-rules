#!/bin/bash
# 🔍 质量检查钩子 - 文件保存后自动进行质量检查
# 基于 quality-manager.sh 改造，适配钩子系统
...
if [[ "$event_type" == "afterFileSave" ]] && [[ -n "$file_path" ]]; then
    echo "🔍 检测到文件保存事件，开始质量检查..." >&2

    # 获取脚本目录
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    CORE_SCRIPT="$SCRIPT_DIR/../../quality/quality-manager.sh"

    # 检查核心脚本是否存在
    if [ ! -f "$CORE_SCRIPT" ]; then
        echo "⚠️  质量管理脚本不存在: $CORE_SCRIPT" >&2
        echo "$input"
        exit 0
    fi
