...
generate_project_identifier() {
    ...
    # 将项目名中的连字符替换为下划线，确保bash变量名有效
    project_name=$(echo "$project_name" | tr '-' '_' | tr '[:upper:]' '[:lower:]')

    local project_hash=$(echo "$PROJECT_ROOT" | md5sum 2>/dev/null | cut -d' ' -f1 | head -c 8)

    # 如果md5sum不可用，使用basename + timestamp
    if [ -z "$project_hash" ]; then
        project_hash=$(echo "${project_name}$(date +%s)" | head -c 8)
    fi

    # 生成项目标识符: 项目名_哈希 (全部小写，确保bash兼容)
    PROJECT_IDENTIFIER="${project_name}_${project_hash}"

    # 生成项目显示名称 (保持原始格式用于显示)
    PROJECT_DISPLAY_NAME=$(basename "$PROJECT_ROOT")

    # 验证项目标识符的唯一性
    if [[ "${DEBUG:-0}" == "1" ]]; then
        echo "🏷️  项目标识符: $PROJECT_IDENTIFIER"
...
# .cursorGrowth 内部路径变量定义 (项目隔离，优化结构)
# 格式: 项目标识符_目录名_DIR
export "${PROJECT_IDENTIFIER}_AI_DIR"="$CURSOR_GROWTH/ai"
export "${PROJECT_IDENTIFIER}_ANALYTICS_DIR"="$CURSOR_GROWTH/analytics"
export "${PROJECT_IDENTIFIER}_ANALYTICS_CACHE_DIR"="$CURSOR_GROWTH/analytics/cache"
export "${PROJECT_IDENTIFIER}_BACKUPS_DIR"="$CURSOR_GROWTH/backups"
export "${PROJECT_IDENTIFIER}_CACHE_DIR"="$CURSOR_GROWTH/cache"
export "${PROJECT_IDENTIFIER}_COMPRESSION_DIR"="$CURSOR_GROWTH/compression"
export "${PROJECT_IDENTIFIER}_CONFIG_DIR"="$CURSOR_GROWTH/config"
export "${PROJECT_IDENTIFIER}_CONVERSATIONS_DIR"="$CURSOR_GROWTH/conversations"
export "${PROJECT_IDENTIFIER}_DEBUG_DIR"="$CURSOR_GROWTH/debug"
export "${PROJECT_IDENTIFIER}_GROWTH_DIR"="$CURSOR_GROWTH/growth"
export "${PROJECT_IDENTIFIER}_INTEGRATIONS_DIR"="$CURSOR_GROWTH/integrations"
export "${PROJECT_IDENTIFIER}_LOGS_DIR"="$CURSOR_GROWTH/logs"
export "${PROJECT_IDENTIFIER}_MONITORING_DIR"="$CURSOR_GROWTH/monitoring"
export "${PROJECT_IDENTIFIER}_PERCEPTION_DIR"="$CURSOR_GROWTH/perception"
export "${PROJECT_IDENTIFIER}_PROJECT_DATA_DIR"="$CURSOR_GROWTH/project_data"
export "${PROJECT_IDENTIFIER}_SYNC_DIR"="$CURSOR_GROWTH/sync"
export "${PROJECT_IDENTIFIER}_USER_DATA_DIR"="$CURSOR_GROWTH/user_data"

# 兼容性变量 (保留常用变量名，用于当前项目)
export AI_DIR="$CURSOR_GROWTH/ai"
export ANALYTICS_DIR="$CURSOR_GROWTH/analytics"
export ANALYTICS_CACHE_DIR="$CURSOR_GROWTH/analytics/cache"
export BACKUPS_DIR="$CURSOR_GROWTH/backups"
export CACHE_DIR="$CURSOR_GROWTH/cache"
export COMPRESSION_DIR="$CURSOR_GROWTH/compression"
export CONFIG_DIR="$CURSOR_GROWTH/config"
export CONVERSATIONS_DIR="$CURSOR_GROWTH/conversations"
export DEBUG_DIR="$CURSOR_GROWTH/debug"
export GROWTH_DIR="$CURSOR_GROWTH/growth"
...
export SYNC_DIR="$CURSOR_GROWTH/sync"
