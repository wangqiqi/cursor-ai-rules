...
show_help() {
    ...
    echo ""
    if [ -f "$CURSOR_DIR/hooks.json" ] || [ -f "$SCRIPT_DIR/hooks.json" ]; then
        echo "🔌 检测到完整 .cursor/ 布局 — Rules / Skills / Hooks 可用"
        echo "   运行 bash .cursor/core/init.sh --quickstart 进行自检"
    fi
    echo ""
...
case "${1:-}" in
    ...
    *)
        ...
        echo "   或安装 .cursor/ 扩展层获得完整 CLI 支持"
        echo ""
        ;;
esac
