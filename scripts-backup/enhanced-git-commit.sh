...
generate_standard_commit_message() {
    ...
    if [ "$has_config" = "true" ]; then
        detailed_description="${detailed_description}- 包含配置变更\n"
    fi

    # 添加生成时间戳
    detailed_description="${detailed_description}\nGenerated by Cursor AI Rules v4.3.0 at $(date '+%Y-%m-%d %H:%M:%S')"

    echo -e "${GREEN}✅ 生成详细提交信息: ${NC}$commit_message"
    echo -e "${CYAN}📋 符合规范: ${NC}Conventional Commits + 智能分析"
    echo -e "${YELLOW}📊 详细变更分析:${NC}"
    echo -e "$detailed_description"
