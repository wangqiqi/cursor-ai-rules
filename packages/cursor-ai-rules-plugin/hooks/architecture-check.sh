...
# 🏗️ 架构合规检查钩子 - 文件保存后自动检查架构合规性
# 基于 architecture-compliance-checker.sh 改造，适配钩子系统
...
if [[ "$event_type" == "afterFileSave" ]] && [[ -n "$file_path" ]]; then
    echo "🏗️ 检测到文件保存事件，开始架构合规检查..." >&2

    # 获取脚本目录
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    CORE_SCRIPT="$SCRIPT_DIR/../core/architecture-compliance-checker.sh"

    # 检查核心脚本是否存在
    if [ ! -f "$CORE_SCRIPT" ]; then
        echo "⚠️  架构合规检查脚本不存在: $CORE_SCRIPT" >&2
        echo "$input"
        exit 0
    fi
