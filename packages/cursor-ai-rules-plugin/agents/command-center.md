...
- 若 `.cursorGrowth` 不存在，先运行：`bash .cursor/features/automation/automation/scripts/growth_init.sh`
...
skill-dispatcher (Skill)
    ...
    ↓ 智能匹配技能
    ↓ 检查依赖
    ↓ 加载 `.cursor/skills/<name>/references/full-guide.md`
    ↓ 应用专业知识
    ↓ 返回结果
command-center 整合结果
    ↓ 返回给用户
...
**技能库位置说明**：
- **官方技能包（Cursor 自动发现）**: `.cursor/skills/<name>/SKILL.md` — 共 **45** 个包
- **调度器**: `.cursor/skills/skill-dispatcher/` — 发现、匹配与路由
- **注册表（元数据索引）**: `.cursor/features/skills/skills/registry.json` — `package` / `guide` 指向官方包
